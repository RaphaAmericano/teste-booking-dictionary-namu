import { DictionaryApiResponse } from "../../../domain/entities/Word";

export const ResponseApiWordArray: DictionaryApiResponse[] = [
  {
    word: "bird",
    phonetic: "/bɜd/",
    phonetics: [
      {
        text: "/bɜd/",
        audio: "",
      },
      {
        text: "/bɜːd/",
        audio: "",
      },
      {
        text: "/bɝd/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/bird-us.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=395396",
        license: {
          name: "BY-SA 3.0",
          url: "https://creativecommons.org/licenses/by-sa/3.0",
        },
      },
    ],
    meanings: [
      {
        partOfSpeech: "noun",
        definitions: [
          {
            definition:
              "A member of the class of animals Aves in the phylum Chordata, characterized by being warm-blooded, having feathers and wings usually capable of flight, and laying eggs.",
            synonyms: [],
            antonyms: [],
            example: "Ducks and sparrows are birds.",
          },
          {
            definition: "A man, fellow.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition:
              "A girl or woman, especially one considered sexually attractive.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition: "Girlfriend.",
            synonyms: [],
            antonyms: [],
            example: "Mike went out with his bird last night.",
          },
          {
            definition: "An airplane.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition: "A satellite.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition:
              "A chicken; the young of a fowl; a young eaglet; a nestling.",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: [
          "bloke",
          "chap",
          "guy",
          "avian",
          "fowl",
          "broad",
          "chick",
          "dame",
          "girl",
          "lass",
        ],
        antonyms: [],
      },
      {
        partOfSpeech: "verb",
        definitions: [
          {
            definition:
              "To observe or identify wild birds in their natural environment.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition: "To catch or shoot birds.",
            synonyms: [],
            antonyms: [],
          },
          {
            definition: "To seek for game or plunder; to thieve.",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: [],
        antonyms: [],
      },
    ],
    license: {
      name: "CC BY-SA 3.0",
      url: "https://creativecommons.org/licenses/by-sa/3.0",
    },
    sourceUrls: ["https://en.wiktionary.org/wiki/bird"],
  },
  {
    word: "bird",
    phonetic: "/bɜd/",
    phonetics: [
      {
        text: "/bɜd/",
        audio: "",
      },
      {
        text: "/bɜːd/",
        audio: "",
      },
      {
        text: "/bɝd/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/bird-us.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=395396",
        license: {
          name: "BY-SA 3.0",
          url: "https://creativecommons.org/licenses/by-sa/3.0",
        },
      },
    ],
    meanings: [
      {
        partOfSpeech: "noun",
        definitions: [
          {
            definition: "A prison sentence.",
            synonyms: [],
            antonyms: [],
            example: "He’s doing bird.",
          },
          {
            definition: "A yardbird",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: ["porridge", "stretch", "time"],
        antonyms: [],
      },
      {
        partOfSpeech: "verb",
        definitions: [
          {
            definition: "To bring into prison, to roof.",
            synonyms: [],
            antonyms: [],
          },
        ],
        synonyms: [],
        antonyms: [],
      },
    ],
    license: {
      name: "CC BY-SA 3.0",
      url: "https://creativecommons.org/licenses/by-sa/3.0",
    },
    sourceUrls: ["https://en.wiktionary.org/wiki/bird"],
  },
  {
    word: "bird",
    phonetic: "/bɜd/",
    phonetics: [
      {
        text: "/bɜd/",
        audio: "",
      },
      {
        text: "/bɜːd/",
        audio: "",
      },
      {
        text: "/bɝd/",
        audio:
          "https://api.dictionaryapi.dev/media/pronunciations/en/bird-us.mp3",
        sourceUrl: "https://commons.wikimedia.org/w/index.php?curid=395396",
        license: {
          name: "BY-SA 3.0",
          url: "https://creativecommons.org/licenses/by-sa/3.0",
        },
      },
    ],
    meanings: [
      {
        partOfSpeech: "noun",
        definitions: [
          {
            definition:
              "The vulgar hand gesture in which the middle finger is extended.",
            synonyms: ["the finger"],
            antonyms: [],
          },
        ],
        synonyms: ["the finger"],
        antonyms: [],
      },
    ],
    license: {
      name: "CC BY-SA 3.0",
      url: "https://creativecommons.org/licenses/by-sa/3.0",
    },
    sourceUrls: ["https://en.wiktionary.org/wiki/bird"],
  },
];
